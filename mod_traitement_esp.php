<?php
	...
	class Controller{
					...
					if(!empty($arrets_zone)){
						...
						for($j=0; $j<=MAX_DISTANCE; $j = $j + $ds_inc){
							for($i=0; $i<count($arrets_zone); $i++){
								...
								$val1 = $arrets_zone[$i][2];
								$val2 = $arrets_zone[$i][3];
								if(GPS::distance($lat_ref, $lng_ref, doubleval($val1), doubleval($val2)) - $j <= 10){
									$near_stop_id = $i;
					...
					if($near_stop_id != -1){
						...
						for($ii = 0; $ii<count($all_arrets); $ii++){
							...
							//	echo "arret " . $arrets_allee[$ii][1] . "\n";
								$val = $all_arrets[$ii][5];
								$distance_restante += doubleval($val); 
								$nb++;
							//}
						}
					}
		...
		static function getNearBus($base, $id_ligne, $sens){
			...
			$term1 = $terminus[1];
			$term2 = $terminus[2];
			$lat_ref = doubleval($term1);
			$lng_ref = doubleval($term2);
			...
			if(!empty($all_bus)){
				...
				for($j=0; $j<=20000; $j = $j + $ds_inc){
						for($i=0; $i<count($all_bus); $i++){
							...
							$val1 = $all_bus[$i][3];
							$val2 = $all_bus[$i][4];
							if(GPS::distance($lat_ref, $lng_ref, doubleval($val1), doubleval($val2)) - $j <= $ds_inc){
								$near_bus_id = $i;
